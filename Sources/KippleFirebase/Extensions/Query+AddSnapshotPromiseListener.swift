...
public extension Query {
    ...
    private func fulfill<T>(_ promise: Future<[T], Error>.Promise, snapshot: QuerySnapshot?, error: Error?) where T: Decodable {
        if let error = error {
            promise(.failure(error))
        }

        let result = Result {
            try snapshot?.documents.compactMap { try $0.data(as: T.self) }
        }
